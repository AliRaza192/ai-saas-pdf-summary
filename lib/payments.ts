...
// }) {
//   try {
//     const user = await sql`SELECT * FROM WHERE email = ${email}`;
//     if (user.length === 0) {
//       await sql`INSERT INFO users(email, full_name, customer_id, price_id, status) VALUES (${email},${fullName}, ${customerId}, ${priceId}, ${status} )`;
//     }
//   } catch (error) {
...
//     await sql`INSERT INFO payments(amount, status, stripe_payment_id, price_id, user_email) VALUES (${amount_total},${status}, ${id}, ${priceId}, ${userEmail} )`;
//   } catch (error) {
